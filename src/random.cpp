...
random_gen::random_gen(double k ) : rand_uniform_dist(0.,1.), rand_gamma_dist(k, 1.0/k) /* gamma dist. of unit mean*/ { 
  seed = ran_dev();
  //seed = 12345 ;  // avoid the randomness by turning on this one (only for check purpose) ... :)
  ran_generator = std::unique_ptr<std::mt19937>(new std::mt19937(seed));
}
...
double random_gen::rand_gamma() {
    return rand_gamma_dist(*ran_generator);
}
